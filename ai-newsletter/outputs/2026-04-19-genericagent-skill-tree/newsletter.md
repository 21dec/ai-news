# GenericAgent: 3,000줄 시드 코드에서 자라나는 자기 진화형 스킬 트리
...
![GenericAgent Memory Architecture](genericagent-skill-tree_diagram_1.png)

### 5계층 메모리 아키텍처

GenericAgent의 핵심 설계는 컨텍스트에 올라가는 정보를 계층별로 분리하는 것입니다.

- **L0 Meta Rules**: 행동 제약 규칙. "사용자 확인 없이 파일을 삭제하지 않는다" 같은 불변 규칙이 위치합니다.
- **L1 Insight Index**: 최소 라우팅 레이어. 어떤 스킬이 존재하는지 인덱스만 유지하며, 전체 스킬 본문을 컨텍스트에 올리지 않습니다.
- **L2 Global Facts**: 축적된 환경 지식. OS 종류, 설치된 패키지, 네트워크 구성 등 재사용 가능한 사실을 저장합니다.
- **L3 Task Skills/SOPs**: 결정화된 워크플로우. 이전 작업에서 검증된 실행 경로가 재사용 가능한 스킬로 기록됩니다.
- **L4 Session Archive**: 세션 요약. 장기 회상을 위해 과거 작업 기록을 압축 저장합니다.
...
![Skill Crystallization Flow](genericagent-skill-tree_diagram_2.png)
