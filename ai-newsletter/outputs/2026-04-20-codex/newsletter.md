# Codex, 코드 생성기를 넘어 개발 워크스테이션으로 확장
...
- **Computer use**: 로컬 컴퓨터와 앱을 조작하는 기능입니다.
- **In-app browsing**: 앱 내부에서 문서와 웹 리소스를 탐색하는 기능입니다.
- **Image generation**: UI 목업, 자산 초안, 문서용 이미지를 생성하는 기능입니다.
- **Memory**: 반복 작업의 선호 설정과 작업 맥락을 유지하는 기능입니다.
- **Plugins**: 외부 도구와 서비스 연결을 확장하는 기능입니다.
...
문제는 모델의 코드 품질만이 아닙니다. **실행 컨텍스트가 분리되어 있다는 점**입니다. 모델은 코드를 쓰지만, 실제 작업은 운영체제 이벤트, 브라우저 상태, 로컬 파일, 앱 UI, 팀 도구 연결 위에서 진행됩니다. 따라서 반자동 수준을 넘기 어렵고, 복붙과 컨텍스트 전환 비용이 누적됩니다.

### 어떻게 해결하는가

이번 업데이트는 Codex를 `text-in/text-out` 인터페이스에서 **action-capable agent**로 이동시키는 방향으로 읽힙니다. 공개된 기능 설명 기준으로 보면 아키텍처는 다음과 같이 이해할 수 있습니다.
...
![2026-04-20-codex_diagram_1.png](2026-04-20-codex_diagram_1.png)
...
- **컨텍스트 전환 감소**: IDE, 브라우저, 문서, 자산 생성 도구를 오가는 비용을 줄입니다.
- **반복 작업 자동화**: 재현, 수정, 테스트, 요약 보고까지 한 흐름으로 묶을 수 있습니다.
- **비API 도구 대응**: GUI 조작 기반이면 API가 없는 환경에도 적용 가능성이 있습니다.
- **개인화된 작업 축적**: 메모리를 통해 팀 규칙과 개인 선호를 지속적으로 반영할 수 있습니다.

### 체크포인트

- **권한 경계**: computer use는 편의성과 함께 오조작 리스크를 동반합니다. 파일 시스템, 브라우저 세션, 사내 시스템 접근 범위를 세밀하게 통제해야 합니다.
- **감사 가능성**: 어떤 문서를 읽고 어떤 액션을 실행했는지 추적 로그가 필요합니다.
- **재현성**: GUI 기반 자동화는 화면 상태와 앱 버전에 민감할 수 있습니다. 안정적인 작업 단위 정의가 중요합니다.
- **메모리 관리**: 장기 메모리는 생산성을 높이지만, 오래된 규칙이나 민감 정보가 누적될 수 있습니다. TTL, 검토, 삭제 정책이 필요합니다.

### 왜 지금 중요한가

코드 생성 자체는 이미 보편화 단계에 들어섰습니다. 다음 경쟁 축은 코드 품질 단독이 아니라 **작업 완결성**으로 이동하고 있습니다. 즉, "좋은 함수를 쓰는가"보다 "문제를 재현하고, 수정하고, 테스트하고, 결과를 남기는가"가 더 중요한 기준이 됩니다.
